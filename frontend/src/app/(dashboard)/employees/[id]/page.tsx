...
export default function EditEmployeePage() {
  ...
  const onSubmit = async (data: FormData) => {
    ...
    try {
      ...
      setApiError(e.response?.data?.message || 'Failed to update employee');
    }
  };
    ...
    <div className="p-8 max-w-lg space-y-6">
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="bg-white rounded-xl shadow-sm p-6 space-y-4">
        ...
        <div>
          ...
          <input {...register('phone')} type="tel" className="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-400" />
